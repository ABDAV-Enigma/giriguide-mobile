...
export const getAllTransactionsByUserId = createAsyncThunk(
  ...
  async (
    { userId, listStatus, userRole, page = 1, size = 40 },
    { rejectWithValue }
  ) => {
    try {
      const response = await axiosInstance.get(
        `/transactions/history?page=${page}&size=${size}&userId=${userId}&status=${listStatus}&role=${userRole}`
      );
...
const transactionSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(
        getTransactionHistoryByTransactionId.fulfilled,
        (state, action) => {
      ...
      .addCase(
        getTransactionHistoryByTransactionId.rejected,
        (state, action) => {
          state.status = "failed";
          state.error = action.payload;
        }
